...
void LveCamera::setPerspectiveProjection(float fovy, float aspect, float near, float far) {
  ...
  // projectionMatrix = glm::perspectiveO
}
...
void LveCamera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up) {
  assert(position != target && "Position should not be the same as target");
  setViewDirection(position, target - position, up);
}

void LveCamera::setViewYXZ(glm::vec3 position, glm::quat rotation) {
  ...
  glm::mat4 T0 = glm::translate(glm::mat4{1.0f}, -position);
  glm::mat4 R = glm::toMat4(glm::inverse(rotation));
